...
const buttons=Object.fromEntries(['sprintButton','crouchButton','jumpButton','fireButton'].map(k=>[k,element()])),input={x:0,y:0},canvas=element(),stick=element(),knob=element();
...
let looked=0;
const controls=sandbox.window.KarlstadControls({canvas,stick,knob,input,active:()=>true,fireEnabled:()=>true,onLook(dx){looked+=dx},onJump(){},onGesture(){}});
...
stick.emit('pointerdown',{pointerId:1,clientX:58,clientY:20});
buttons.fireButton.emit('pointerdown',{pointerId:3,clientX:200,clientY:100});assert(input.fire);assert(input.y>0,'shoot while moving');
buttons.fireButton.emit('pointermove',{pointerId:3,clientX:230,clientY:100});assert(looked>0,'fire drag must aim');
buttons.fireButton.emit('pointerup',{pointerId:4});assert(input.fire,'unrelated touch must not release trigger');
buttons.fireButton.emit('pointercancel',{pointerId:3});assert(!input.fire);assert(input.y>0,'trigger release must preserve movement');
buttons.fireButton.emit('pointerdown',{pointerId:3,clientX:200,clientY:100});controls.reset();assert(!input.fire);assert.equal(input.y,0);
console.log('PASS: joystick, pointer ownership, movement + fire + aim, cancellation, crouch, accessible state and reset');
