...
window.KarlstadControls=function({canvas,stick,knob,input,active,onLook,onJump,onGesture}){
 let stickId=null,lookId=null,origin=null,look=null,sprintId=null;
 ...
 function releaseStick(e){if(e&&e.pointerId!==stickId)return;stickId=null;origin=null;input.x=input.y=0;input.autoSprint=false;knob.style.transform='';stick.classList.remove('pressed','sprinting')}
 for(const ev of ['pointerup','pointercancel','lostpointercapture'])stick.addEventListener(ev,releaseStick);
 canvas.addEventListener('pointerdown',e=>{if(!active()||lookId!==null||e.button>0)return;onGesture();lookId=e.pointerId;look={x:e.clientX,y:e.clientY};canvas.setPointerCapture(e.pointerId);if(e.pointerType==='mouse'&&!matchMedia('(pointer:coarse)').matches){try{canvas.requestPointerLock?.()?.catch?.(()=>{})}catch{}}});
 ...
 for(const ev of ['pointerup','pointercancel','lostpointercapture'])canvas.addEventListener(ev,e=>{if(e.pointerId===lookId){lookId=null;look=null}});
 ...
 function reset(){releaseStick();lookId=null;look=null;sprintId=null;input.sprint=false;input.crouch=false;sprint.classList.remove('pressed');duck.classList.remove('pressed');duck.setAttribute('aria-pressed','false')}
